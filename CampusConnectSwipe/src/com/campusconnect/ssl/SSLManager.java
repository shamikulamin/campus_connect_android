...
public class SSLManager {
	...
	{		
		try{
			...
			entity.addPart("enc_user",new StringBody(enc_user));
			entity.addPart("enc_pass",new StringBody(enc_pass));
			...
			if( directoryType == IMAGE_DIRECTORY_TYPE ) {
				File vDir = new File(sDirectory);
				if(vDir.isDirectory())
				{
					int iNumOfImages = 0;
					...
					for(int i = 0; i < iNumOfImages; i++)
					{
						...
						entity.addPart("image", new InputStreamBody(new FileInputStream(vImage), vImage.getName()));
					}
				}
			} else if( directoryType == RECORDING_DIRECTORY_TYPE ) {
				File vRec = new File(sDirectory);
				if( vRec.isFile() ) {
					entity.addPart("recording", new InputStreamBody(new FileInputStream(vRec), vRec.getName()));
				}
			}
	...
	public boolean sendCredentials(String enc_uid, String enc_pass) throws ConnectTimeoutException, UpdateException {
		try {
			...
			String version = pInfo.versionName + "_ANDROID";
